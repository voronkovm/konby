...
function resolveTaskWorkspaceDir(task, boardDir) {
  const workspace = task && task.data && task.data.workspace
    ? String(task.data.workspace)
    : '';
  const out = workspace || boardDir;
  if (path.isAbsolute(out)) return out;
  return path.join(boardDir, out);
}
...
function renderBoard(stateRef, options = {}) {
  ...
  lines.push('Hotkeys for a task: m - move | c - open VS Code | t - open tmux | p - PR | g - merge | x - archive');
  lines.push('');
  ...
  if (stateRef.mode === 'add') {
    lines.push('');
    lines.push('Add task popup (Enter save, Esc cancel, ←/→ move cursor):');
    lines.push(renderEditableLine('description: ', stateRef.addBuffer, stateRef.addCursor));
  }
...
function clampLineCursor(buffer, cursor) {
  const text = String(buffer || '');
  if (!Number.isFinite(cursor)) return text.length;
  return Math.max(0, Math.min(text.length, cursor));
}

function insertLineEditText(buffer, cursor, text) {
  const current = String(buffer || '');
  const pos = clampLineCursor(current, cursor);
  const value = String(text || '');
  return {
    buffer: `${current.slice(0, pos)}${value}${current.slice(pos)}`,
    cursor: pos + value.length,
  };
}

function deleteLineEditBackward(buffer, cursor) {
  const current = String(buffer || '');
  const pos = clampLineCursor(current, cursor);
  if (pos === 0) return { buffer: current, cursor: pos };
  return {
    buffer: `${current.slice(0, pos - 1)}${current.slice(pos)}`,
    cursor: pos - 1,
  };
}

function deleteLineEditForward(buffer, cursor) {
  const current = String(buffer || '');
  const pos = clampLineCursor(current, cursor);
  if (pos >= current.length) return { buffer: current, cursor: pos };
  return {
    buffer: `${current.slice(0, pos)}${current.slice(pos + 1)}`,
    cursor: pos,
  };
}

function moveLineEditCursor(buffer, cursor, delta) {
  return clampLineCursor(buffer, clampLineCursor(buffer, cursor) + delta);
}

function renderEditableLine(label, buffer, cursor) {
  const text = String(buffer || '');
  const pos = clampLineCursor(text, cursor);
  const cursorChar = pos < text.length ? text[pos] : ' ';
  const afterCursor = pos < text.length ? text.slice(pos + 1) : '';
  return `${label}${text.slice(0, pos)}${ansiInverse(cursorChar)}${afterCursor}`;
}
